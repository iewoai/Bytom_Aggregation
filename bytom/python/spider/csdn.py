...
class csdn:
	def __init__(self):
		...
		self.path = os.path.dirname(os.path.abspath(__file__))+'/../lib/csdn.p'
		self.data = {}
		self.data_new = {}
	...
	def get_more(self, url):
		...
		author= s.xpath('//a[@id="uid"]/text()')[0]
		...
		# 缩略文字
		info = text[:100]
		return title,c_time,int(views),int(comments),int(likes),collections,author,avatar_url,len(images_num),tag,text,info
			...
			if len(urls) != 0:
				for url in set(urls):
					...
					if not (csdn_id in self.data):
						num += 1
						title,c_time,views,comments,likes,collections,author,avatar_url,images_num,tag,text,info = self.get_more(url)
